...
def simulate_take_order(
    ...
    # ── 关键修复：检查 remove_time ──
    # 真实 take_order 执行流程：空驶 → sync_time → 等装货窗 → sync_time → 干线 → sync_time → remove_by_id
    # 如果 remove_time < ready_time，货源在等待装货期间就会被 sync_time_minutes 从 online 池移除
    # 导致最后 remove_by_id 返回 None → "cargo_id 已失效"
    # 因此必须检查 remove_time >= finish_time（整个执行期间货源都必须在线）
    remove_time_str = cargo.get("remove_time")
    if remove_time_str:
        try:
            remove_minutes = wall_time_to_simulation_minutes(str(remove_time_str))
            if remove_minutes < finish_time:
                return OrderSimulation(
                    cargo_id=cargo_id, pickup_km=pickup_km, haul_km=0, total_km=pickup_km,
                    end_lat=end_lat, end_lng=end_lng,
                    pickup_minutes=pickup_minutes, wait_at_pickup_minutes=wait_at_pickup, haul_minutes=0,
                    total_minutes=pickup_minutes + wait_at_pickup, finish_time=ready_time,
                    price_yuan=price_yuan, cost_yuan=0, net_profit=0,
                    profit_per_hour=0, feasible=False,
                    infeasible_reason="cargo_will_expire_during_execution",
                )
        except (ValueError, TypeError):
            pass
