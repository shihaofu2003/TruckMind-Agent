...
MAX_EXTRA_QUERIES = 0  # 每轮最多额外查询次数（从2改为0，减少query时间消耗）
...
class Planner:
    ...
    def _evaluate_candidates(
        ...
        # 评分
        scored = score_all_candidates(candidates, lambda_hour)
        # 过滤
    ...
    def _select_query_points(
        ...
        # 情况二：当前无好货，查最佳热区
        if not evals or evals[0].utility <= 0:
            current_lat = float(status.get("current_lat", 0))
            current_lng = float(status.get("current_lng", 0))
        ...
        # 情况三：top1 终点未查过且收益高
            ...
            top1 = evals[0]
            if top1.utility > lambda_hour * 2:  # 收益较高
                end_lat = top1.simulation.end_lat
                end_lng = top1.simulation.end_lng
                if end_lat != 0 or end_lng != 0:
                    points.append((end_lat, end_lng))
